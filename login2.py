...
def handle_admin_features():
    ...
    uploaded_file = st.file_uploader(get_text("upload_file"), type=["pdf", "doc", "docx", "txt"])
    if uploaded_file is not None:
        st.write(f"{get_text('uploaded_file')}: {uploaded_file.name}, Type: {uploaded_file.type}")
    ...
    if question and openai_api_key:
        ...
        if "admin_uploaded_files" in st.session_state:
            for uploaded_file in st.session_state["admin_uploaded_files"]:
                file_content += uploaded_file.getvalue().decode("utf-8") + "\n"
        else:
            try:
